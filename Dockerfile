...
RUN echo 'Dreamsky124' | vncpasswd -f > $HOME/.vnc/passwd
...
RUN echo "su -l -c 'vncserver :2000 -geometry 1360x768' "  >>/Dreamsky124.sh

RUN echo 'cd /noVNC-1.2.0' >>/Dreamsky124.sh

RUN echo './utils/launch.sh  --vnc localhost:7900 --listen 8900 ' >>/Dreamsky124.sh

RUN chmod 755 /Dreamsky124.sh
